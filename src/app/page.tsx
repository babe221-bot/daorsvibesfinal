    ...
    <AppLayout>
      <div className="relative w-full h-full min-h-screen">
        <video 
          ...
          data-ai-hint="logo transformation video"
        >
          <source src="/logo-transformation-video.mp4" type="video/mp4" />
          Your browser does not support the video tag.
        </video>
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent z-10"></div>
        <Header className="relative z-20" /> {/* Ensure header is above the video and overlay */}
        <main className="relative z-20 flex-1 p-4 md:p-8 space-y-8">
                  ...
                  {recentPlaylists.map(playlist => (
                    <div key={playlist.name} className="flex items-center gap-4 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">Otkrijte svoju sljedeću omiljenu pjesmu.</p>
                      </div>
                    </div>
                  ))}
                </div>
